...
def setup_model_and_optimizer(model_provider_func):
    ...
    if args.deepspeed:
        ...
        if args.pipe_parallel_size > 0:
            if hasattr(model.module, '_megatron_batch_fn'):
                model.set_batch_fn(model.module._megatron_batch_fn)
            if hasattr(model.module, '_input_grad'):
                model.set_input_grad(model.module._input_grad)
            if hasattr(model.module, '_input_type'):
                model.set_input_type(model.module._input_type)
            if hasattr(model.module, '_input_pipe_partitioned'):
                model.set_input_pipe_partitioned(model.module._input_pipe_partitioned)
