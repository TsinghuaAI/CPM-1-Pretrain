...
from .language_model import get_seq2seq_language_model
...
from megatron.model.transformer_seq2seq import ParallelTransformerLayerPipe
from .language_model import Seq2SeqEmbeddingPipe

import tds as deepspeed
from deepspeed.pipe import PipelineModule, LayerSpec, TiedLayerSpec


def t5_attention_mask_func(attention_scores, ltor_mask):
...
class T5Model(MegatronModule):
    ...
    def __init__(self, num_tokentypes=0, parallel_output=True):
        ...
        self.language_model, self._language_model_key = get_seq2seq_language_model(
    ...
    def forward(self, 
                enc_token_ids, enc_position_ids, enc_attention_mask, 
                dec_token_ids=None, dec_position_ids=None, dec_attention_mask=None, cross_attention_mask=None,
                labels=None,
        ...
        lm_output = self.language_model(
                      ...
                      enc_tokentype_ids=enc_tokentype_ids, dec_tokentype_ids=dec_tokentype_ids,
                      layer_past=layer_past, get_key_value=get_key_value)

        if get_key_value:
            lm_output, presents, cross_presents = lm_output
...
class T5ModelPipe(PipelineModule, MegatronModule):
    ...
    def __init__(self, num_tokentypes=0, parallel_output=True, add_pooler=False, topology=None):
        ...
        self.specs.append(lambda x: (x[0].transpose(0,1).contiguous(), x[1], 
                                     x[2].transpose(0,1).contiguous(), x[3], x[4]))
        ...
        for x in range(args.num_layers):
            self.specs.append(
                LayerSpec(ParallelTransformerLayerPipe,
                          ...
                          init_method=self.init_method,
                          output_layer_init_method=self.output_layer_init_method,
                          layer_number=x + args.num_layers,
                          is_cross_flag = True))

        # Undo data format change and drop mask
        self.specs.append(lambda x: x[2].transpose(0,1).contiguous())
